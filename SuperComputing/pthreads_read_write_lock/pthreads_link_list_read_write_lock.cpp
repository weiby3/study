...
#include "my_rand.h"
#include "timer.h"
#include "my_read_write_lock.h"

using namespace std;

// comment it to become write_prefer
//#define READ_PREFER

#ifdef READ_PREFER
my_read_write_lock_read_prefer my_lock;
#else
my_read_write_lock_write_prefer my_lock;
#endif
...
int Insert(int value) {
    ...
    if (curr == NULL || curr->data > value) {
        temp = static_cast<list_node_s *>(malloc(sizeof(struct list_node_s)));
        temp->data = value;
        temp->next = curr;
        if (pred == NULL)
            head = temp;
