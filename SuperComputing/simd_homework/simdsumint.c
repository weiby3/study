...
#ifdef INTRIN_AVX2
int32_t sumint_avx2(const int32_t* pbuf, size_t cntbuf)
{
    int32_t s = 0;    // 求和变量.
    size_t i;
    size_t nBlockWidth = 8;    // 块宽. AVX寄存器能一次处理8个int32_t.
    size_t cntBlock = cntbuf / nBlockWidth;    // 块数.
    size_t cntRem = cntbuf % nBlockWidth;    // 剩余数量.
    __m256i xidSum = _mm256_setzero_si256();    // 求和变量。[AVX2] PXOR. 赋初值0.
    __m256i xidLoad;    // 加载.
    const __m256i* p = (const __m256i*)pbuf;    // SSE批量处理时所用的指针.
    const int32_t* q;    // 单个数据处理时所用指针.

    // AVX批量处理.
    for(i=0; i<cntBlock; ++i)
    {
        xidLoad = _mm256_load_si256(p);    // [AVX2] MOVDQA. 加载.
        xidSum = _mm256_add_epi32(xidSum, xidLoad);    // [AVX2] PADDD. 32位整数紧缩环绕加法.
        p ++;
    }
    // 合并.
    q = (const int32_t*)&xidSum;
...
// 32位整数数组求和_AVX四路循环展开版.
int32_t sumint_avx2_4loop(const int32_t* pbuf, size_t cntbuf)
{
    int32_t s = 0;    // 返回值.
    size_t i;
    size_t nBlockWidth = 8*4;    // 块宽. SSE寄存器能一次处理8个int32_t，然后循环展开4次.
    size_t cntBlock = cntbuf / nBlockWidth;    // 块数.
    size_t cntRem = cntbuf % nBlockWidth;    // 剩余数量.
    __m256i xidSum = _mm256_setzero_si256();    // 求和变量。[AVX2] PXOR. 赋初值0.
    __m256i xidSum1 = _mm256_setzero_si256();
    __m256i xidSum2 = _mm256_setzero_si256();
    __m256i xidSum3 = _mm256_setzero_si256();
    __m256i xidLoad;    // 加载.
    __m256i xidLoad1;
    __m256i xidLoad2;
    __m256i xidLoad3;
    ...
    {
        xidLoad = _mm256_load_si256(p);    // [AVX2] MOVDQA. 加载.
        xidLoad1 = _mm256_load_si256(p+1);
        xidLoad2 = _mm256_load_si256(p+2);
        xidLoad3 = _mm256_load_si256(p+3);
        xidSum = _mm256_add_epi32(xidSum, xidLoad);    // [AVX2] PADDD. 32位整数紧缩环绕加法.
        xidSum1 = _mm256_add_epi32(xidSum1, xidLoad1);
        xidSum2 = _mm256_add_epi32(xidSum2, xidLoad2);
        xidSum3 = _mm256_add_epi32(xidSum3, xidLoad3);
        p += 4;    // 四路循环展开.
    }
    // 合并.
    xidSum = _mm256_add_epi32(xidSum, xidSum1);    // 两两合并(0~1).
    xidSum2 = _mm256_add_epi32(xidSum2, xidSum3);    // 两两合并(2~3).
    xidSum = _mm256_add_epi32(xidSum, xidSum2);    // 两两合并(0~3).
    q = (const int32_t*)&xidSum;
    s = q[0] + q[1] + q[2] + q[3] + q[4] + q[5] + q[6] + q[7];
...
#endif    // #ifdef INTRIN_SSE2
#ifdef INTRIN_AVX2
    if (simd_avx_level(NULL) >= SIMD_AVX_2)
    {
        runTest("sumint_avx2", sumint_avx2);    // 32位整数数组求和_AVX版.
        runTest("sumint_avx2_4loop", sumint_avx2_4loop);    // 32位整数数组求和_AVX四路循环展开版.
    }
#endif    // #ifdef INTRIN_AVX2
